...
# Offline self‑distillation (Zhang et al., arXiv:2604.01193)
self_distill = False               # run offline SSD after main training
ssd_iters = 1000                   # number of SSD fine‑tuning steps
ssd_gen_len = 128                  # length of generated completion per prompt
ssd_prompt_len = 512               # length of the prompt (taken from real data)
ssd_batch_size = 12                # micro‑batch size during SSD (can be smaller)
distill_temp = 0.8                 # temperature for generation
top_p = 0.95                       # nucleus sampling parameter
ssd_lr = 1e-5                      # learning rate during SSD (usually lower)
# -----------------------------------------------------------------------------
...
# top‑p filtering helper (used in SSD generation)
@torch.no_grad()
def top_p_filter(logits, p):
    ...
    # scatter the sorted indices back to the original order
...
if self_distill:
    ...
    if ssd_gen_len <= 0:
        ...
        for ssd_iter in range(ssd_iters):
            ...
            with torch.no_grad():
                gen_seq = prompts
                for _ in range(ssd_gen_len):
                    logits, _ = raw_model(gen_seq)
                    next_logits = logits[:, -1, :] / distill_temp
                    filtered = top_p_filter(next_logits.clone(), top_p)
            ...
            if grad_clip != 0.0:
                ...
                torch.nn.utils.clip_grad_norm_(model.parameters(), grad_clip)
            scaler.step(ssd_optimizer)
            scaler.update()
            ssd_optimizer.zero_grad(set_to_none=True)

            if ssd_iter % log_interval == 0 and master_process:
                print(f"ssd iter {ssd_iter}: loss {loss_ssd.item():.4f}")

        if master_process:
            loss_after = estimate_loss()
